...
const LoginRegister = () => {
        ...
        if (email === '' || !email || password === '' || !password) {
            ...
            if (login) {
                ...
                if (cpassword != password) {
                    ...
                    axios.post('http://localhost:8096/api/register', { name: name, email: email, password: password, cpassword: cpassword })
                        .then(res => {
    ...
    return (
        <div className='pt-5 bg-white min-vh-100'>
            <div className='container d-flex justify-content-center align-content-center'>
                <form className='p-2 border bg-light shadow border rounded' style={{ minHeight: '270px', minWidth: "320px" }} onSubmit={handleSubmit}>
                    ...
                    <center className={`${msg_type === 'error' ? 'fs-5 fw-semibold text-danger' : ''}`}>{msg ? msg : ''}</center>
                    {
